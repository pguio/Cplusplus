...
private:
  ...
  enum { boolean=1, character, string,
...
#endif
{
  ...
  Parser::insertOption(boolean, "--boolean", boolean,
                       "Parse a boolean", Any(Boolean));
  Parser::insertOptionAlias(boolean,"boolean");

  Parser::insertOption(character, "--character", character,
                       "Parse a character", Any(Character));
  Parser::insertOptionAlias(character, "character");

  Parser::insertOption(string, "--string", charStr,
                       "Parse a string", Any(String));
  Parser::insertOptionAlias(string,"string");

  Parser::insertOption(integer, "--integer", integer,
                       "Parse an integer", Any(Integer));
  Parser::insertOptionAlias(integer,"integer");

  Parser::insertOption(int_enum, "int_enum", integer,
                       "Parse an int_enum", Any(Enum));

  Parser::insertOption(real32, "real32", real,
                       "Parse a float", Any(Float));

  Parser::insertOption(real64, "real64", real,
                       "Parse a double", Any(Double));

  Parser::insertOption(vectbool, "vectbool", boolVect,
                       "Parse a vector of boolean", Any(VectBool));

  Parser::insertOption(vectchar, "vectchar", charVect,
                       "Parse a vector of character", Any(VectChar));

  Parser::insertOption(vectstr, "vectstr", stringVect,
                       "Parse a vector of string", Any(VectStr));

  Parser::insertOption(vectint, "vectint", intVect,
                       "Parse a vector of integer", Any(VectInt));

  Parser::insertOption(vectdouble, "vectdouble", realVect,
                       "Parse an array of double", Any(VectDouble));

  Parser::insertOption(dirlist, "-I", charStr,
                       "Parse include directories",Any(DirList));

#if defined(HAVE_BLITZ)
  Parser::insertOption(realvec3, "realvec3", realVect,
                       "Parse blitz::TinyVector<float,3>",Any(RealVec3));

  Parser::insertOption(realarray1d, "realarray1d", realVect,
                       "Parse blitz::Array<float,1>",Any(RealArray1d));
#endif
}
...
void Foo::parseParameters()
{
  ...
  if (Parser::parseOption(int_enum,Enum)) {
    Parser::checkMap(int_enum, Enum_map, Enum);
  ...
  if (Parser::parseOptions(dirlist,DirList))
